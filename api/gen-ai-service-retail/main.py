...
@app.route('/service/ai/bot', methods=['POST'])
def universal_chat():
    vertexai.init(project=os.environ.get("PROJECT_ID"), location=os.environ.get("VERTEX_REGION"))
    ...
    context = "You are an assistant for a prestigious airline company. Your primary role is to provide helpful and friendly service to customers, specifically addressing their queries about flight delays. Your responses should always be empathetic, understanding the inconvenience that flight delays can cause to passengers."
...
def get_product_by_description():
    try:
        ...
        if not request_data or 'query' not in request_data or 'user_persona' not in request_data:
            return jsonify({"error": "Search query or user persona not provided."}), 400  # Return JSON response

        search_query = request_data['query'].lower().split()
        user_persona = request_data['user_persona']
        ...
        query = client.query(kind=kind)
        query.add_filter('user_persona', '=', user_persona)
        user_products = list(query.fetch())

        if not user_products:
            return jsonify({"error": f"No products found for user persona: {user_persona}"}), 404  # Return JSON response

        user_products_with_scores = []

        for product in user_products:
            product_words = product['search_query'].lower().split()
            score = sum(1 for term in search_query if term in product_words)

            if score > 0:
                user_products_with_scores.append((product, score))

        user_products_with_scores.sort(key=lambda x: x[1], reverse=True)

        if not user_products_with_scores:
            return jsonify({"error": f"No matching products found for query: {search_query} and user persona: {user_persona}"}), 404  # Return JSON response

        user_products = [dict(result[0]) for result in user_products_with_scores]

        return jsonify(user_products)  # Return the user products as JSON
...
@app.route('/service/product/persona_description', methods=['POST'])
def get_product_by_persona_description():
    try:
        request_data = request.get_json()
...
@app.route('/store_conversation_data', methods=['POST'])
def store_conversation_data():
    try:
        ...
        store_data_in_datastore(conversation_id, like, dislike, user_message, bot_message)
...
def store_data_in_datastore(conversation_id, like, dislike, user_message, bot_message):
    # Create a new entity for the Datastore
    entity = datastore.Entity(key=datastore_client.key('ChatData', conversation_id))
    entity.update({
        'like': like,
        'dislike': dislike,
...
@app.route('/get_conversation_data/<conversation_id>', methods=['GET'])
def get_conversation_data(conversation_id):
    try:
        # Retrieve the conversation from Datastore
        conversation = retrieve_data_from_datastore(conversation_id)
...
def retrieve_data_from_datastore(conversation_id):
    # Retrieve the entity from Datastore
    key = datastore_client.key('ChatData', conversation_id)
    entity = datastore_client.get(key)

    if entity:
        # Return the entity properties as a dictionary
        return {
            'conversationId': conversation_id,
            'like': entity.get('like', 0),
            'dislike': entity.get('dislike', 0),
            'userMessage': entity.get('userMessage', ''),
            'botMessage': entity.get('botMessage', ''),
        }
    else:
...
def store_conversation_data():
        try:
            ...
            user_message = data.get('userMessage', '')
            bot_message = data.get('botMessage', '')
            action = data.get('action')  # Added to handle like/dislike
            ...
            # Handle like/dislike
            if action:
                update_like_dislike_in_datastore(conversation_id, action)
...
def update_like_dislike_in_datastore(conversation_id, action):
        # Retrieve the entity from Datastore
        key = datastore_client.key('ChatData', conversation_id)
        entity = datastore_client.get(key)
        ...
        if entity:
            ...
            if action == 'like':
                entity['like'] = entity.get('like', 0) + 1
            elif action == 'dislike':
                entity['dislike'] = entity.get('dislike', 0) + 1

            # Save the updated entity to the Datastore
            datastore_client.put(entity)
...
from langchain.vectorstores import MatchingEngine  # Assuming the import
from langchain.embeddings import VertexAIEmbeddings
embeddings = VertexAIEmbeddings()

# Define Matching Engine as Vector Store
ME_EMBEDDING_DIR = "embeddings_test_1"
me = MatchingEngine.from_components(
    project_id="gbg-neuro",
    region="europe-west2",
    gcs_bucket_name= "embeddings_test_1",
    embedding=embeddings,
    index_id="projects/534348290993/locations/europe-west2/indexes/520306494449647616",
    endpoint_id="projects/534348290993/locations/europe-west2/indexEndpoints/8188810759954759680",
)
